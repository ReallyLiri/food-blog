...
export const fullScreenAnimation = (ref_canvas: HTMLCanvasElement) => {
  ...
  function rgb(col: number) {
    col += 0.000001;
    const r = parseInt(((0.5 + Math.sin(col) * 0.5) * 16).toString());
    const g = parseInt(((0.5 + Math.cos(col) * 0.5) * 16).toString());
    const b = parseInt(((0.5 - Math.sin(col) * 0.5) * 16).toString());
  ...
  function rgbArray(col: number) {
    col += 0.000001;
    const r = parseInt(((0.5 + Math.sin(col) * 0.5) * 256).toString());
    const g = parseInt(((0.5 + Math.cos(col) * 0.5) * 256).toString());
    const b = parseInt(((0.5 - Math.sin(col) * 0.5) * 256).toString());
    return [r, g, b];
  }
  function colorString(arr: number[]) {
    const r = parseInt(arr[0].toString());
    const g = parseInt(arr[1].toString());
    const b = parseInt(arr[2].toString());
    return (
      "#" +
  ...
  function process(lets: letsOptions) {
    ...
    let p = Math.atan2(lets.camX, lets.camZ);
    let d = Math.sqrt(lets.camX * lets.camX + lets.camZ * lets.camZ);
    d -= Math.sin(lets.frameNo / 80) / 25;
    const t = Math.cos(lets.frameNo / 300) / 165;
  ...
  function spawnParticle(lets: letsOptions) {
    const pt: pointOption = {};
    const p = Math.PI * 2 * Math.random();
    const ls = Math.sqrt(Math.random() * lets.distributionRadius);
    pt.x = Math.sin(p) * ls;
